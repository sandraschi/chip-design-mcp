...
const BACKEND_NAME: &str = "chip-design-mcp-backend.exe";
const BACKEND_PORT: u16 = 11022;

fn dev_backend_path() -> Option<PathBuf> {
    if !cfg!(debug_assertions) { return None; }
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("binaries")
        .join("chip-design-mcp-backend-x86_64-pc-windows-msvc.exe");
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) = app.path().resolve("resources/chip-design-mcp-backend.exe", BaseDirectory::Resource) {
        tried.push(path.display().to_string());
        if path.exists() { return Ok(path); }
    }
